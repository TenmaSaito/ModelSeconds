	...
	/*** 頂点バッファの設定 ***/
		...
		for (int nCntField = 0; nCntField < MAX_FIELD; nCntField++)
		{
			...
			if (g_aField[nCntField].bUse == true)
			{
				...
				/*** テクスチャ座標の設定 ***/
				pVtx[0].tex = D3DXVECTOR2(0.0f, 0.0f);
				pVtx[1].tex = D3DXVECTOR2(1.0f * g_aField[nCntField].nXBlock, 0.0f);
				pVtx[2].tex = D3DXVECTOR2(0.0f, 1.0f * g_aField[nCntField].nZBlock);
				pVtx[3].tex = D3DXVECTOR2(1.0f * g_aField[nCntField].nXBlock, 1.0f * g_aField[nCntField].nZBlock);
			}
